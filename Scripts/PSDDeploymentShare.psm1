...
.NOTES
        ...
        Version - 0.1.3 - (PC) - Changed Get-PSDContent to output destination path always unless specified Passthru parameter
...
function Get-PSDConnection{
    ...
    elseif ($global:psddsDeployRoot -like "\\*") {
        ...
        try{
            ...
            #do nothing; assume mapped already
        }
...
function Get-PSDProvider{
    ...
    If(-not($Result = Get-PSDrive Deploymentshare -ErrorAction SilentlyContinue)){
...
#region FUNCTION [Get-PSDContent] finding and retrieving the specified content.
function Get-PSDContent{
    ...
    <#
    The source location specifies a relative path within the deployment share.
    The destination specifies the local path where the content should be copied.
    If no destination is specified, it will be copied to a cache folder.
    #>
    $dest = ""
    ...
    if ($PSBoundParameters.ContainsKey('destination')) {
        Write-PSDLog -Message "$($MyInvocation.MyCommand.Name): Parameters: Content=[$content], destination=[$destination]"
        $dest = $destination
        Write-PSDLog -Message "$($MyInvocation.MyCommand.Name): Dest is now: $dest"
    }
    else{
        Write-PSDLog -Message "$($MyInvocation.MyCommand.Name): Parameters: Content=[$content]"
        Write-PSDLog -Message "$($MyInvocation.MyCommand.Name): Destination is blank, running [Get-PSDLocalDataPath]"
    ...
    elseif ($global:psddsDeployRoot -ilike "http*"){
        ...
        $Files = Get-PSDContentWeb -content $content -destination $dest
    }
    elseif ($global:psddsDeployRoot -like "\\*"){
        Write-PSDLog -Message "$($MyInvocation.MyCommand.Name): global:psddsDeployRoot is now: $global:psddsDeployRoot"
        Write-PSDLog -Message "$($MyInvocation.MyCommand.Name): Running [Get-PSDContentUNC -content $content -destination $dest]"
        $Files = Get-PSDContentUNC -content $content -destination $dest
    }
    else{
...
function Get-PSDContentUNC
{
    ...
    $result = Copy-PSDFolder "$($global:psddsDeployRoot)\$content" $destination
    return $result
}
...
function Get-PSDContentWeb{
    ...
    if($tsenv:BranchCacheEnabled -eq "YES"){
        if(-Not([String]::IsNullOrEmpty($tsenv:SMSTSDownloadProgram))){
...
if (Test-Path "tsenv:")
{
    if ($tsenv:DeployRoot -ne "")
    {
        ...
        {
            $Null = Get-PSDConnection -deployRoot $tsenv:DeployRoot -username $tsenv:UserID -password $tsenv:UserPassword
        }
    }
}
