...
Function Get-PSDLocalInfo{
    ...
    Get-WmiObject Win32_ComputerSystem | % {
      $tsenv:Make = $_.Manufacturer.Trim()
      $tsenv:Model = $_.Model.Trim()
      $tsenv:Memory = [int] ($_.TotalPhysicalMemory / 1024 / 1024)
    }

    Get-WmiObject Win32_ComputerSystemProduct | % {
      $tsenv:UUID = $_.UUID.Trim()
      $tsenv:CSPVersion = $_.Version.Trim()
    }

    Get-CimInstance -ClassName MS_SystemInformation -NameSpace root\WMI | % {
      $tsenv:MSSIBaseBoardProduct = $_.BaseBoardProduct.Trim()
      $tsenv:MSSISystemSku = $_.SystemSku.Trim()
    }


    Get-WmiObject Win32_BaseBoard | % {
      $tsenv:Product = $_.Product.Trim()
    }
    ...
    # TEST: Battery
    Write-PSDLog -Message "$($MyInvocation.MyCommand.Name): IsOnBattery"
    ...
    switch ($Win32_ComputerSystem.model){
        ...
        "Parallels Virtual Platform"{$tsenv:IsVM = "True"}
        "Standard PC (Q35 + ICH9, 2009)"{$tsenv:IsVM = "True"}
        "Standard PC (i440FX + PIIX, 1996)"{$tsenv:IsVM = "True"}
