...
if ($tsInProgress){
    ...
    if ($tsenv:SkipWizard -ine "YES"){
        Write-PSDLog -Message "PSDDirty is now: true"
        $tsenv:PSDDirty = $true
        ...
        {
            Write-PSDLog -Message ("$($MyInvocation.MyCommand.Name): Running [Show-PSDWizard -ResourcePath {0} -Passthru]" -f $PSDWizardPath)
            $result = Show-PSDWizard -ResourcePath $PSDWizardPath -Passthru
            if ($result -eq $false){
                Write-PSDLog -Message ("$($MyInvocation.MyCommand.Name): Error: {0}, aborting..." -f $result)
                Show-PSDInfo -Message ("Error: {0}, aborting..." -f $result) -Severity Information -OSDComputername $OSDComputername -Deployroot $global:psddsDeployRoot
                Stop-PSDLogging
        ...
        Else{
            Write-PSDLog -Message ("$($MyInvocation.MyCommand.Name): {0} path not found, aborting..." -f $PSDWizardPath)
