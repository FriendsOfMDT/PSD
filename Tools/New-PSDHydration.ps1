...
.DESCRIPTION
	 This script installs the ADK, installs the PE, installs MDT, and then imports and extends the PSD environment for it. Once completed it installs and configures IIS for the PSD
...
.Example
	...
	This runs the script with the verbose command and logs the output to the screen.
...
process
{
    ...
    $objLabel.Location = New-Object System.Drawing.Size(10,20) 
    $objLabel.Size = New-Object System.Drawing.Size(315,20) 
    $objLabel.Text = "Please enter a username with access to the share:"
    $objForm.Controls.Add($objLabel) 
...
process
{
     if(!(Test-Path -Path "$($PSScriptRoot)\ExportedData.CSV" ))
     {
          ...
          Elseif(-not(($RequiredfileTest | Where-Object {$_.Name -like "Windows Assessment Toolkit*"}).Count -ge 3))
          {
               ...
               {
                    Write-PSDInstallLog -Message "The user voluntarily exited the program. Now breaking."
                    break
               }
          }
          ...
          {
               Write-PSDInstallLog -Message "Something went wrong during the installation of the ADK. Please validate the exit code: $($ADKProcess.ExitCode)" -LogLevel 3
               break
          }
          Write-PSDInstallLog -Message "Succesfully installed the ADK. Now moving on to install the WinPE add-in if needed."
          #Starting ADK PE Install IF The Path was true 
          if($ADKPEPath)
          {
               Write-PSDInstallLog -Message "The ADKPE IS required. Now starting the PE add-in installation."
               ...
               {
                    Write-PSDInstallLog -Message "Something went wrong during the installation of the ADK. Please validate the exit code: $($ADKPEProcess.ExitCode)" -LogLevel 3
                    break
               }
          ...
          {
               Write-PSDInstallLog -Message "Something went wrong during the installation of MDT. Please validate the exit code: $($MDT.Process.ExitCode)"
               break
          }
          ...
          Write-PSDInstallLog -Message "We have now completed the BASE install. Now hydrating with ISO files. Requesting information from the user."
...
PSDDebug=YES
"@
          Write-PSDInstallLog "Setting bootstrap.ini file to a variable. Now forcing overwrite in the PSD generated share."
...
"@
          ...
          Write-PSDInstallLog -Message "Sucessfully reached the IIS installation and configuration section. You must reboot. Please re-run this script with the configure option." -LogLevel 2
